...
from models.driver import Ubication, Post
...
async def login_for_access_token(
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Incorrect username or usernamepassword",
            headers={"WWW-Authenticate": "Bearer"},
        )
...
def login(plate: str):
    try:
        ...
        resultados_json = conn_adempiere.login_driver(plate)
        ...
        conn_adempiere.close_connection()

@app.post('/ubicacion')
def save_ubi(ubicacion: Ubication):
    #codigo que se insertara en la base de datos nevada
    try:
        conn_nevada = NevadaConnect()
        conn_nevada.connect()
        result  = conn_nevada.query_data(ubicacion)
        if result == 1:
            return "Se insertaron los datos Correctamente"
        else:
            return HTTPException(status_code=404, detail="Error al insertar datos") 
    finally:
        conn_nevada.close_connection()